...
function MyRoomRequestListener() {
    this.onSubscribeRoomDone = function (event) {
        if (event.result == resultcode_success) {
            ...
            if (event.roomdata.id == jungle_roomId) {
                // inside jungle
        ...
        if (updateevent.update.length == 1) {
            ...
            if (current_role == role_robber) {
                if (updateevent.update[0] == 0) {
                    //document.getElementById('game_state').innerHTML = "You lost.You are a slow robber";
                    displayGameResultAndRestartButton("You lost.You are a slow robber.Press to start a new game");
                }
                else {
                    displayGameResultAndRestartButton("You won.You are a fast robber.Press to start a new game");
                }
            }
            else {
                if (updateevent.update[0] == 0) {

                    displayGameResultAndRestartButton("You won.You are a fast policeman.Press to start a new game");
                }
                else {
                    displayGameResultAndRestartButton("You lost.You are a slow policeman.Press to start a new game");
                }
            }
            gameOver();
            return;
