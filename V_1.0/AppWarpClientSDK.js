...
(function() {
   ...
   // Message
	...
	this.updateRoomProperties = function(roomID, tableProperties, removeArray)
	{
		...
		var bytes = buildWarpRequest(update_room_property,JSON.stringify(params));
		socket.send(bytes.buffer);	
	}
	...
	this.joinRoomWithNUser = function(count){
		...
		var bytes = buildWarpRequest(join_room_n_user,JSON.stringify(params));
		socket.send(bytes.buffer);	
	}
	...
	this.joinRoomWithProperties = function(properties)
	{
		...
		var bytes = buildWarpRequest(join_room_with_properties,JSON.stringify(params));
		socket.send(bytes.buffer);	
	}
	...
	this.getRoomWithNUser = function(count){
		...
		var bytes = buildWarpRequest(get_room_with_n_user,JSON.stringify(params));
		socket.send(bytes.buffer);	
	}
	...
	this.getRoomWithProperties = function(properties)
	{
		...
		var bytes = buildWarpRequest(get_room_with_properties,JSON.stringify(params));
		socket.send(bytes.buffer);	
	}
	...
	function handleNotify(bytearray, startIndex){
		...
		if(notify.updateType == room_property_change){
			var payLoadString = bin2String(notify.payLoad);
			var data = new RoomData(payLoadString);
			var payLoadObj = JSON.parse(payLoadString);
			...
			var count = NotificationObservers.length;		
			for( var i = 0; i < count; i++ ){
			   NotificationObservers[i].onUserChangeRoomProperty(data, payLoadObj.user, JSON.parse(payLoadObj.properties));
			}
		}	
	...
	function handleResponse(bytearray, startIndex){
        ...
        if(response.requestType == request_type_join_room || response.requestType == join_room_n_user || response.requestType == join_room_with_properties){
	   ...
	   if(response.requestType == request_type_chat){
        ...
        if(response.requestType == request_type_update_peers){
		 ...
		 if(response.requestType == update_room_property){
			handleUpdatePropertyResponse(response);
		 }	
		 if(response.requestType == get_room_with_n_user || response.requestType == get_room_with_properties){
...
//MatchMaking
var join_room_n_user = 24;
var update_room_property = 25;
var join_room_with_properties = 27;
var get_room_with_n_user = 28;
var get_room_with_properties = 29;
var max_property_size_bytes = 2048;
var room_property_change = 11;
