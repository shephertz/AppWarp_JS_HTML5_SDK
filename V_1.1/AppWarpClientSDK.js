...
(function() {
	  ...
	  xmlhttp.onreadystatechange=function()
	  {
		if (xmlhttp.readyState==4 && xmlhttp.status==200)
		{
			...
			for(var key in res)
			{
				if(key == "address")
				{
					socket = new WebSocket("ws://"+res.address+":12346");
					...
					socket.onopen = function(){	  
						...
						else
						{
							...
							for( var i = 0; i < count; i++ ){
							   ConnectionRequestObservers[i].onConnectDone( resultcode_bad_request);	
							}
						}
					}
				  ...
				  socket.onmessage = that.onMessage;
				}
			}
		}
		else if(xmlhttp.readyState==4 && xmlhttp.status == 404)
		{
			var count = ConnectionRequestObservers.length;		
			for( var i = 0; i < count; i++ ){
			   ConnectionRequestObservers[i].onConnectDone( resultcode_api_not_found );	
			}
		}
	  }
	  xmlhttp.open("GET","http://control.appwarp.shephertz.com/lookup?api="+AppWarpApiKey,true);
	  xmlhttp.send();
	...
	// Zone
	this.createRoom = function(name, owner, max, properties) {
		 var params = {};
		 params.name = name;
		 params.owner = owner;
		 params.maxUsers = max;
		 ...
		 if(properties)
		 {
			 properties_str = JSON.stringify(properties);
			 if(properties_str.length > max_property_size_bytes)
			 {
				var event = new RoomEvent(result_size_error, "");
    ...
    this.sendPrivateChat = function(to, msg) {
         if(msg.length >= 512){
            var count = ConnectionRequestObservers.length;      
            for( var i = 0; i < count; i++ ){
               ChatRequestObservers[i].onSendChatDone( resultcode_bad_request );    
         ...
         params.chat= msg;      
         var payloadString = JSON.stringify(params);    
         var bytes = buildWarpRequest(request_type_private_chat, payloadString); 
         socket.send(bytes.buffer); 
    };
	...
	this.addNotificationListener = function(observer){
		if(!observer.onRoomCreated || !observer.onRoomDestroyed || !observer.onUserLeftRoom ||
		   ...
		   !observer.onChatReceived || !observer.onUpdatePeersReceived || !observer.onUserChangeRoomProperty ||
           !observer.onPrivateChatReceived){
	...
	this.addRoomRequestListener = function(observer){
		if(!observer.onSubscribeRoomDone || !observer.onUnsubscribeRoomDone || !observer.onJoinRoomDone ||
		   !observer.onLeaveRoomDone || !observer.onGetLiveRoomInfoDone || !observer.onSetCustomRoomDataDone || !observer.onUpdatePropertyDone ||
           !observer.onLockPropertiesDone || !observer.onUnlockPropertiesDone){
	...
	this.addZoneRequestListener = function(observer){
		if(!observer.onCreateRoomDone || !observer.onDeleteRoomDone || !observer.onGetAllRoomsDone ||
		   !observer.onGetOnlineUsersDone || !observer.onGetLiveUserInfoDone || !observer.onSetCustomUserInfoDone || !observer.onGetMatchedRoomsDone){
	...
	this.addConnectionRequestListener = function(observer){
		if(!observer.onConnectDone || !observer.onDisconnectDone){
	...
	this.addChatRequestListener = function(observer){
		if(!observer.onSendChatDone || !observer.onSendPrivateChatDone){
   ...
   // Message
	...
	this.updateRoomProperties = function(roomID, tableProperties, removeArray)
	{
		...
		var params = {};
		params.id = roomID;
		params.addOrUpdate = tableProperties;
		params.remove = removeProperties;

		var bytes = buildWarpRequest(request_type_update_room_property,JSON.stringify(params));
		socket.send(bytes.buffer);	
	}
	
	this.joinRoomWithNUser = function(count){
		var params = {};
		params.userCount = count;

		var bytes = buildWarpRequest(request_type_join_room_n_user,JSON.stringify(params));
		socket.send(bytes.buffer);	
	}
	
	this.joinRoomWithProperties = function(properties)
	{
		var params = {};
		params.properties = properties;

		var bytes = buildWarpRequest(request_type_join_room_with_properties,JSON.stringify(params));
		socket.send(bytes.buffer);	
	}

    this.joinRoomInRange = function(minUsers, maxUsers, maxPreferred)
    {
        var params = {};
        ...
        var bytes = buildWarpRequest(request_type_join_room_in_range,JSON.stringify(params));
        socket.send(bytes.buffer);  
    }
	
    this.getRoomInRange = function(minUsers,maxUsers)
    {
        var params = {};
        params.minUsers = minUsers;
        params.maxUsers = maxUsers;

        var bytes = buildWarpRequest(request_type_get_room_in_range,JSON.stringify(params));
        socket.send(bytes.buffer);  
    }

	this.getRoomWithNUser = function(count){
		var params = {};
		params.userCount = count;

		var bytes = buildWarpRequest(request_type_get_room_with_n_user,JSON.stringify(params));
		socket.send(bytes.buffer);	
	}
	
	this.getRoomWithProperties = function(properties)
	{
		var params = {};
		params.properties = properties;

		var bytes = buildWarpRequest(request_type_get_room_with_properties,JSON.stringify(params));
		socket.send(bytes.buffer);	
	}
    ...
    this.lockProperties = function(properties)
    {
        ...
        var bytes = buildWarpRequest(request_type_lock_properties,JSON.stringify(params));
        socket.send(bytes.buffer);  
    }
    ...
    this.unlockProperties = function(properties)
    {
        ...
        var bytes = buildWarpRequest(request_type_unlock_properties,JSON.stringify(params));
        socket.send(bytes.buffer);  
    }
	...
	function handleNotify(bytearray, startIndex){
		var notify = buildNotify(bytearray, startIndex);
         ...
         //var payLoad = bin2String(notify.payLoad);
        ...
        if(notify.updateType == update_type_private_chat){
            var payLoadString = bin2String(notify.payLoad);
            var payLoadObj = JSON.parse(payLoadString);
            
            var count = NotificationObservers.length;       
            for( var i = 0; i < count; i++ ){
               NotificationObservers[i].onPrivateChatReceived(payLoadObj.sender, payLoadObj.chat);
            }
        }       
		
		if(notify.updateType == update_type_room_property_change){
			var payLoadString = bin2String(notify.payLoad);
			var data = new RoomData(payLoadString);
			var payLoadObj = JSON.parse(payLoadString);
			...
			var count = NotificationObservers.length;		
			for( var i = 0; i < count; i++ ){
			   NotificationObservers[i].onUserChangeRoomProperty(payLoadObj.sender, JSON.parse(payLoadObj.properties), JSON.parse(payLoadObj.lockProperties));
			}
		}	
	...
	function handleResponse(bytearray, startIndex){
         ...
         //var payLoad = bin2String(response.payLoad);
         //var payLoadObj = JSON.parse(payLoad);
         //console.log(payLoadObj);
        ...
        if(response.requestType == request_type_join_room || response.requestType == request_type_join_room_n_user || response.requestType == request_type_join_room_with_properties || response.requestType == request_type_join_room_in_range){
	   ...
	   if(response.requestType == request_type_chat){
         ...
         if(response.requestType == request_type_private_chat){
            handleSendPrivateChatResponse(response);
         }
        if(response.requestType == request_type_update_peers){
         ...
         if(response.requestType == request_type_lock_properties){
            handleLockPropertiesResponse(response);
         }
         if(response.requestType == request_type_unlock_properties){
            handleUnLockPropertiesResponse(response);
         }
	...
	function handleAuthResponse(authResponse){
	    ...
	    var count = ConnectionRequestObservers.length;		
	    for( var i = 0; i < count; i++ ){
		   ConnectionRequestObservers[i].onConnectDone( authResponse.resultCode );	
		}
	}
   ...
   // Lobby Response
	...
	function handleUpdatePropertyResponse(roomResponse)
	{
		var event = new LiveRoomInfoEvent(roomResponse.resultCode, roomResponse.payLoad);
	    var count = RoomRequestObservers.length;		
	    for( var i = 0; i < count; i++ ){
		   RoomRequestObservers[i].onUpdatePropertyDone(event);
		}
	}
	
	function handleGetRoomWithRequest(response)
	{
		var event = new MatchedRoomsEvent(response.resultCode, response.payLoad);
		var count = RoomRequestObservers.length;		
	    for( var i = 0; i < count; i++ ){
		   ZoneRequestObservers[i].onGetMatchedRoomsDone(event);
		}
	}

    function handleLockPropertiesResponse(response)
    {
        var count = RoomRequestObservers.length;        
        for( var i = 0; i < count; i++ ){
           RoomRequestObservers[i].onLockPropertiesDone(response.resultCode);
        }
    }
	
    function handleUnLockPropertiesResponse(response)
    {
        var count = RoomRequestObservers.length;        
        for( var i = 0; i < count; i++ ){
           RoomRequestObservers[i].onUnlockPropertiesDone(response.resultCode);
        }
    }
    ...
    function handleSendPrivateChatResponse(chatResponse){
        var count = ChatRequestObservers.length;        
        for( var i = 0; i < count; i++ ){
           ChatRequestObservers[i].onSendPrivateChatDone(chatResponse.resultCode);
        }
    }
	// Update Response
...
function LiveRoomInfoEvent(code, payload) {
	...
	if(code == resultcode_success){		
		...
		this.properties = payLoadObj.properties;
...
function MatchedRoomsEvent(code, payload){
	this.result = code;
	if(code == resultcode_success){		
		var payLoadString = bin2String(payload);
		var payLoadObj = JSON.parse(payLoadString);
		...
		for(var key in payLoadObj){
			var room = {};
			room.id = key;
			room.name = payLoadObj[key].name;
			room.owner = payLoadObj[key].owner;
			room.maxUsers = payLoadObj[key].maxUsers;
...
var resultcode_api_not_found = 7;
...
//MatchMaking
var request_type_join_room_n_user = 24;
var request_type_update_room_property = 25;
var request_type_join_room_with_properties = 27;
var request_type_get_room_with_n_user = 28;
var request_type_get_room_with_properties = 29;
var max_property_size_bytes = 2048;
var update_type_room_property_change = 11;
var result_size_error = 7;
var request_type_join_room_in_range = 37;
var request_type_get_room_in_range = 38;

//Properties
var request_type_lock_properties = 35;
var request_type_unlock_properties = 36;

var request_type_keep_alive = 63;
var request_type_private_chat = 30;
var request_type_move = 31;
var assoc_port = 64;

var update_type_private_chat = 12;
var update_type_move_completed = 13;
